...
function apt_software {
    ...
    # Install Terraform on Ubuntu Machine
...
function user_setup {
    ...
    # cat /etc/group |grep -w tools &>/dev/null || sudo groupadd $GROUP_NAME
    ...
    for users in $username; do
        ...
        cat /etc/passwd | awk -F: '{print $1}' | grep -w $users &>/dev/null ||  useradd $users
    ...
    ## Create master-user and configure SSH keys
